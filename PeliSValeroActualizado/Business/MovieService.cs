            ...
            if (peliculaSeleccionada != null && peliculaSeleccionada.InStock)
            {
                ...
                if (respuesta.Equals("Si", StringComparison.OrdinalIgnoreCase))
                {
                    if (cuentaExistente.Balance >= peliculaSeleccionada.Price)
                    {
                        ...
                        cuentaExistente.peliculasCompradas.Add(peliculaSeleccionada.Title);

                        DateTime fechaActual = DateTime.Now;

                        var entrada = new Historial
                        {
                            Cantidad = cuentaExistente.Balance,
                            Fecha = fechaActual,
                            Pelicula = peliculaSeleccionada.Title,
                            Titular = cuentaExistente.FirstName + " " + cuentaExistente.LastName,
                            Proveedor = "PeliSValero",
                            Estado = "Pagado"
                        };
                        cuentaExistente.historialCuenta.Add(entrada);
